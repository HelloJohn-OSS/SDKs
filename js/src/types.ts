...
/**
 * Public bot-protection configuration returned by /v2/auth/config.
 * Only the siteKey is exposed — the secretKey never leaves the server.
 */
export interface BotProtectionConfig {
    enabled: boolean;
    provider: string;                // "turnstile"
    siteKey: string;                 // Cloudflare Turnstile public site key
    protectLogin: boolean;
    protectRegistration: boolean;
    protectPasswordReset: boolean;
    appearance?: string;             // "always" | "execute" | "interaction-only"
...
export interface RegisterOptions {
    ...
    /** Cloudflare Turnstile token — required when bot protection is enabled for registration. */
...
export interface AuthConfig {
    ...
    /** Bot protection public config. Present only when enabled for this tenant/client. */
