<template>
  <div class="hj-sign-in">
    ...
    <form class="hj-sign-in__form" @submit.prevent="handleSubmit">
      ...
      <button
        class="hj-sign-in__submit"
        type="submit"
        :disabled="isLoading || (loginTurnstileEnabled && !turnstileToken)"
      >
...
import type { MFARequiredResult, AuthConfig } from "@hellojohn/js";
import { ref, computed, onMounted } from "vue";
import { useAuth } from "../composables/useAuth";
import { useTurnstile } from "../composables/useTurnstile";
import HjTurnstile from "./HjTurnstile.vue";
...
const { loginWithPassword, isLoading, getConfig } = useAuth();
...
// Derived Turnstile properties
const loginTurnstileEnabled = computed(() => {
  const bp = config.value?.bot_protection;
  return !!(bp?.enabled && bp?.protectLogin && bp?.siteKey);
});

const botSiteKey = computed(() => config.value?.bot_protection?.siteKey ?? "");
const botTheme = computed(
  () => (config.value?.bot_protection?.theme as "light" | "dark" | "auto") ?? "auto"
);
const botAppearance = computed(
  () => (config.value?.bot_protection?.appearance as "always" | "execute" | "interaction-only") ?? "always"
);
