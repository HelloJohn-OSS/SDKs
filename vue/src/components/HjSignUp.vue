<template>
  <div class="hj-sign-up">
    ...
    <form class="hj-sign-up__form" @submit.prevent="handleSubmit">
      ...
      <HjTurnstile
        v-if="registrationTurnstileEnabled"
        :site-key="botSiteKey"
        :theme="botTheme"
      ...
      <button
        class="hj-sign-up__submit"
        type="submit"
        :disabled="isLoading || (registrationTurnstileEnabled && !turnstileToken)"
      >
...
import type { AuthConfig } from "@hellojohn/js";
...
const { signUp, isLoading, getConfig } = useAuth();
...
const registrationTurnstileEnabled = computed(() => {
  const bp = config.value?.bot_protection;
  return !!(bp?.enabled && bp?.protectRegistration && bp?.siteKey);
});
