...
export function useAuth() {
  ...
  async function getConfig() {
    return auth.getTenantConfig();
  }

  async function loginWithPassword(email: string, password: string, turnstileToken?: string): Promise<LoginPasswordResult> {
    state.isLoading = true;
    state.error = null;

    try {
      const result = await auth.loginWithPassword(email, password, turnstileToken);
  ...
  async function signUp(email: string, password: string, name?: string, turnstileToken?: string): Promise<SignUpResult> {
    state.isLoading = true;
    state.error = null;
    try {
      const result = await auth.register({ email, password, name, ...(turnstileToken ? { turnstileToken } : {}) });
