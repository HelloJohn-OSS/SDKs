...
export function SignUp({
    ...
    // Turnstile config for registration protection
    const botProtection = config?.bot_protection;
    const registrationTurnstileEnabled = !!(botProtection?.enabled && botProtection?.protectRegistration && botProtection?.siteKey);
    ...
    const renderStep1 = () => (
        <>
            ...
            {/* Turnstile bot-protection widget (step 1 only, last step before submit) */}
            {registrationTurnstileEnabled && !hasCustomFields && (
                <TurnstileWidget
            ...
            <button
                type="submit"
                disabled={submitting || !canSubmitStep1 || (registrationTurnstileEnabled && !hasCustomFields && !turnstileToken)}
                style={{
                    ...buttonStyle,
                    opacity: (submitting || !canSubmitStep1 || (registrationTurnstileEnabled && !hasCustomFields && !turnstileToken)) ? 0.7 : 1,
                    cursor: (submitting || !canSubmitStep1 || (registrationTurnstileEnabled && !hasCustomFields && !turnstileToken)) ? "not-allowed" : "pointer",
                }}
            >
    ...
    const renderStep2 = () => (
        <>
            ...
            {/* Turnstile on the final step when there are custom fields */}
            {registrationTurnstileEnabled && hasCustomFields && (
                <TurnstileWidget
                    siteKey={botProtection!.siteKey!}
                    onVerify={onTurnstileVerify}
            ...
            <div style={{ display: "flex", gap: "12px" }}>
                ...
                <button
                    type="submit"
                    disabled={submitting || (registrationTurnstileEnabled && hasCustomFields && !turnstileToken)}
                    style={{
                        ...buttonStyle,
                        flex: 2,
                        opacity: (submitting || (registrationTurnstileEnabled && hasCustomFields && !turnstileToken)) ? 0.7 : 1,
                        cursor: (submitting || (registrationTurnstileEnabled && hasCustomFields && !turnstileToken)) ? "not-allowed" : "pointer",
                    }}
                >
